...
export const BGLaccountBalanceHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // @ts-ignore
    const [priceInfo, balanceInfo] = await Promise.all([_getBGLPriceChange(), _getBglAddressBalance(wallet.address)])
...
const createBitgesellAccountsHandler = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    for (let i = 0; i <= 9; i++) {
      ...
      // @ts-ignore
      const [priceInfo, balanceData] = await Promise.all([_getBGLPriceChange(), _getBglAddressBalance([address])])
      const balanceUSD = priceInfo.bglPriceUSD * sb.toBitcoin(balanceData.balance)

      accounts.push({ ...account, index: i, balance: { balance: balanceData.balance, usd: balanceUSD } })
    }
...
// TODO: adapt to allow: BSC, Ethereum addresses to hava unified endpoint for transactions
const txHistoryHandler = async (
  req: Request,
  res: Response
) => {
...
const bglTxBuilderHandler = async (
  req: Request,
  res: Response
) => {
  const {
  ...
  if (!to && !from && !amount && !privateKey && !fee) return res.json({ msg: 'Failed: Invalid Request', success: false })
  ...
  try {
    await jsbgl.asyncInit(globalThis)
    // @ts-ignore
    if (!globalThis.isAddressValid(to)) return res.status(400).json({ success: false, error: 'Invalid recpient address' })
...
async function importFromPkeyOrSeedPhraseHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (globalThis.isMnemonicCheckSumValid(seedOrPkey)) {
      ...
      const { address } = account
      // @ts-ignore
      const [priceInfo, balanceInfo] = await Promise.all([_getBGLPriceChange(), _getBglAddressBalance(address)])
      const balanceUSD = priceInfo.bglPriceUSD * sb.toBitcoin(balanceInfo.balance)
      accounts.push({ address, index: index, privateKey: account.privateKey, mnemonic: seedOrPkey, balance: { balance: sb.toBitcoin(balanceInfo.balance), usd: balanceUSD } })
      ...
      const [priceInfo, balanceInfo] = await Promise.all([_getBGLPriceChange(), _getBglAddressBalance(wallet.address)])
      const balanceUSD = priceInfo.bglPriceUSD * sb.toBitcoin(balanceInfo.balance)
      accounts.push({ address: wallet.address, index: 0, privateKey: seedOrPkey, balance: { balance: sb.toBitcoin(balanceInfo.balance), usd: balanceUSD }, mnemonic: null })
...
async function _buildTransactionObject({
  from,
  to,
  fee,
  privateKey,
...
async function _getBglAddressBalance(bglAddress: string) {
...
// @ts-ignore
async function _generateAddress(entropy) {
  // @ts-ignore
...
/// End of Bitaps Explorer API Functions

//// Begin push notification logic
// 1. Received bgl
// 2. pending tx
// 3. received USDT(future version) - Ethereum/Binance Smartchain
// @ts-ignore
...
export {
  ...
  dashboardHandler, importAccountHandler, importFromPkeyOrSeedPhraseHandler,
  importWalletHandler,
  txHistoryHandler
};
